...
description: "As a follow-up to my previous post/posts/dearth-patterns/ where I psuedo-whined about common knowledge gaps in technology"
...
*   Log mentally or in a journal what each finding means.  You should be thinking critically and expecting results as you try things: "if the following test does A then it means X.  If it doesn't then it means Y".  See this [fantastic single-bit-flip troubleshooting session here](http://blog.ksplice.com/2010/06/attack-of-the-cosmic-rays/) where the author systematically dives deeper and deeper into a segfault until he proves that a system binary [bit-flipped](http://en.wikipedia.org/wiki/RAM_parity) while in memory.
...
`export DISPLAY=localhost:0.0` and
```bash
$ gnome-terminal
...
X11 isn't encrypted [by design](http://en.wikipedia.org/wiki/X_Window_System#Network).  So use SSH forwarding.
...
![](/uploads/2010/06/branching_example.png "branching_example")
...
In Git, you should be branching and merging a lot.  There's a great book called [Pro Git](http://progit.org/book/) which [has this](http://progit.org/book/ch3-1.html) and more explanation.  Git really changed the way I thought about developing.  I wouldn't say I'm an expert yet.  I still need to learn to commit more often and branch more.  But I'm ok with these future optimization goals, I'm not ok with manual version control on a file share.  :)
