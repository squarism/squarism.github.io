...
description: "Blog post about splitting a sentence in ruby and keeping the punctuation"
...
> _Update:_ Doh!  You can just do `text.split(/(\.)\s+/)` to make the regex keep the delimiter with the parens group match.  Then you have to join in groups and some other stuff.  I keep the post below for posterity. :)
...
Ok, whatever.  That's regex stuff.  Let's try applying what we've learned from our [last post](/posts/a-different-repl-workflow-in-ruby/) and create an inline rspec test to do what we want.
